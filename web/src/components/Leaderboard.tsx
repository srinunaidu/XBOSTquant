...
function rowToObj(r: BoardRow, ix: number, th: number, cost: number, signalOnly: boolean) {
  ...
  return {
    ...
    wins: Math.round((m.winRate || 0) * (m.totalTrades || 0) / 100),
    losses: (m.totalTrades || 0) - Math.round((m.winRate || 0) * (m.totalTrades || 0) / 100),
    gross: m.grossPreCost ?? (m.netPnL + (m.totalCosts || 0)),
...
const COLS = [
  ...
  { field: 'wins', headerName: 'Wins', width: 66, type: 'rightAligned' },
  { field: 'losses', headerName: 'Loss', width: 66, type: 'rightAligned' },
  { field: 'gross', headerName: 'Gross ₹', width: 104, type: 'rightAligned', valueFormatter: (p: any) => fmtMoney(p.value) },
