...
export interface Engine {
  ...
  todBucket15(t: number): string;
  aggregateProfile(trades: any[]): Record<string, any>;
  classifyCell(cell: any, cfg?: any): { status: string; reasons: string[]; timeConcentrated?: boolean };
  mergeBuckets(cells: Record<string, any>, classified: Record<string, any>): string[][];
  classifyTransfer(fCell: any, oCell: any): string;
  consumeProfile(profile: any, q: any): { decision: string; families: string[]; reason: string };
  profileId(market: string, instrument: string, tf: string, runId: string, dataHash: string): string;
  fnv1a(s: any): string;
